...
#Region "Curve"
    ...
    Shared Function LinInterpol(<ExcelArgument(Description:="Plazos")> _
        ...
        Return Result
    End Function
    <ExcelFunction(category:=Categoria, Description:="Esta funcion hace interpolacion por método de Varianza Total y se aplica a la interpolación de una volatilidad ATM.")> _
    Shared Function ATMInterpol(<ExcelArgument(Description:="Plazos: vector de plazos en d'ias, ordenados de forma ascendente")> _
                                        ByVal plazos As Object, _
                                        <ExcelArgument(Description:="atms: vector de vols atm correspondientes a los plazos del vector de plazos")> _
                                        ByVal atms As Object, _
                                        <ExcelArgument(Description:="plazo: plazo en d'ias al que queremos interpolar")> _
                                        ByVal Plazo As Double) As Double

        'La funcion encuentra entre que vertices del vector plazos esta el argumento plazo y luego interpola
        'a plazo utilizando las volatilidades atm correspondientes a los dos vertices encontrados.
        ...
        Dim plazo1 As Double
        Dim plazo2 As Double
        Dim plazoyear As Double
        Dim plazoyear1 As Double
        Dim plazoyear2 As Double
        Dim Result As Double
        Dim arrayAtms() As Double
        Dim arrayPlazos() As Double
        Dim IndexBase As Integer
        ...
        Try
            ...
            If atms Is Nothing Or plazos Is Nothing Then
                Return 0
            End If

            arrayAtms = Range2Array(Of Double)(atms)
            arrayPlazos = Range2Array(Of Double)(plazos)



            IndexBase = 0
            j = arrayAtms.GetUpperBound(0) - 1
            i = arrayPlazos.GetUpperBound(0) - 1
            ...
            Else
                ...
                Next i
                plazoyear = Plazo / 365
                plazoyear1 = plazo1 / 365
                plazoyear2 = plazo2 / 365


                Result = Math.Sqrt((plazoyear - plazoyear1) / (plazoyear2 - plazoyear1) * plazoyear2 / plazoyear * atm2 ^ 2 + _
                                   (plazoyear2 - plazoyear) / (plazoyear2 - plazoyear1) * plazoyear1 / plazoyear * atm1 ^ 2)
            End If
        Catch ex As Exception
            MsgBox(ex.Message)
        Finally
...
#Region "Swaps"
 ...
 Shared Function pvAnyLegFromCurve(<ExcelArgument(Description:="Fecha val")> _
    ...
    Shared Function pvFixedAmortSwapLegFromCurve(<ExcelArgument(Description:="Fecha val")> _
                                            ...
                                            ByVal periodicity As String, _
                                            <ExcelArgument(Description:="Ajuste de fechas fin de periodo: 0, no ajusta; 1, hábil siguiente (sólo si la fecha no es hábil)")> _
                                            ByVal accAdjustment As String, _
                                            <ExcelArgument(Description:="Ajuste de fechas de pago (respecto a fechas fin de periodo): 0, hábil siguiente (sólo si la fecha no es hábil); 1, hábil siguiente; 2, hábil subsiguiente")> _
                                            ByVal pmtAdjustment As String, _
                                            <ExcelArgument(Description:="Periodo corto: 0, al final; 1, al principio")> _
                                            ByVal typeStubPeriod As Integer, _
                                            <ExcelArgument(Description:="1:bullet, 2:constante, 3:frances, else:bullet")> _
                                            ByVal typeAmortization As Integer, _
...
#Region "BootStrapping"
    ...
    Shared Function ITB2(<ExcelArgument(Description:="Trade date")> ByVal valueDate As Date, _
              ...
              <ExcelArgument(Description:="Periocidad pata fija")> ByVal periodicity As String, _
              <ExcelArgument(Description:="Ajuste fechas fin de periodo: 0, no ajusta; 1, hábil siguiente (sólo si la fecha no es hábil)")> ByVal accrualAdjustment As String, _
              <ExcelArgument(Description:="Ajuste fechas pago (respecto fechas fin de periodo): 0, hábil siguiente (sólo si la fecha no es hábil); 1, hábil siguiente; 2, hábil subsiguiente")> ByVal paymentAdjustment As String, _
        ...
        Try
            ...
            per = Ten(periodicity) 'Se agrega ya que ahora periodicity es string
            auxPeriodsLong = 1 + auxPeriods - Ten(arraySwapTenors(cuantosSwaps - 1)) / per 'OJO
            ...
            For i = 1 To cuantosSwaps
                ...
                dp = 1.0E+20
                nAux = 0.0
                auxPeriods = CType(Ten(arraySwapTenors(i - 1)) / per + auxPeriodsLong, Integer)
    ...
    Shared Function ITBCross(<ExcelArgument(Description:="Trade date")> _
                         ...
                         ByVal accrualAdjustment As String, _
                         <ExcelArgument(name:="paymentAdj", Description:="Ver floatCalendar(")> _
                         ByVal paymentAdjustment As String, _
...
#Region "Spread TAB"
    ...
    Private Shared Function MyPi() As Double
        Dim Pi As Double
    ...
    Private Shared Function CND(ByVal z As Double) As Double
        ...
        y = 1 / (1 + 0.2316419 * Math.Abs(z))
        r = (1 / Math.Sqrt(2 * Pi) * Math.Exp(-0.5 * z ^ 2)) * y * (a(1) + y * (a(2) + y * (a(3) + y * (a(4) + y * a(5)))))
        If z > 0 Then r = 1 - r
...
#Region "AUX"
    ...
    End Function
    <ExcelFunction(category:=Categoria, Description:="Devuele 1 si n=m, si no 0")> _
    Shared Function flag(<ExcelArgument(Description:="n")> ByVal n As Object, _
                                     <ExcelArgument(Description:="m")> ByVal m As Object) As Integer
        Dim cell1 As Microsoft.Office.Interop.Excel.Range
        Dim cell2 As Microsoft.Office.Interop.Excel.Range
        Dim result As Integer
    ...
    Shared Function calendarAmortize(<ExcelArgument(Description:="Fecha inicio")> ByVal startDate As Date, _
                                    <ExcelArgument(Description:="Fecha fin")> ByVal endDate As Date, _
                                    <ExcelArgument(Description:="Periocidad")> ByVal tenor As String, _
                                    <ExcelArgument(Description:="Ajuste fechas fin de periodo: 0, no ajusta; 1, hábil siguiente (sólo si la fecha no es hábil)")> ByVal accAdjustment As String, _
                                    <ExcelArgument(Description:="Ajuste fechas de pago (respecto a fechas fin de periodo): 0, hábil siguiente (sólo si la fecha no es hábil); 1, hábil siguiente; 2, hábil subsiguiente")> ByVal pmtAdjustment As String, _
        ...
        periodicity = Ten(tenor) 'Incorporado por ASC para que la periocidad se un tenor
        ...
        Try
            Select Case typeStubPeriod
                ...
                Case 3
                    ...
                    i = 1
                    While auxDate >= startDate 'ASC cabio > por >= para consistencia con caso sin periodo chueco
                        auxDate = AddMonths(endDate, -i * periodicity)
                        i = i + 1
                    End While
                    ...
                    If numberOfPeriods < 2 Then 'Incorporado por ASC para controlar 1 periodo largo
                        numberOfPeriods = 1
                        ReDim aux(0 To numberOfPeriods, 0 To 5)
                        aux(1, 1) = startDate
                        aux(1, 2) = endDate
                    Else
                        ReDim aux(numberOfPeriods, 5)
                ...
                Case 4
                    auxDate = startDate
                    i = 1
                    While auxDate <= endDate 'ASC cabio < por <= para consistencia con caso sin periodo chueco
                        auxDate = AddMonths(startDate, i * periodicity)
                        i = i + 1
                    End While
    ...
    <ExcelFunction(category:=Categoria, Description:="Genera calendario de pata variable de un Swap")> _
        ...
        periodicity = Ten(Tenor) 'Incorporado por ASC para que la periocidad sea un tenor
        ...
        Try
            Select Case typeStubPeriod
                ...
                Case 3
                    ...
                    If numberOfPeriods < 2 Then 'Incorporado por ASC para controlar 1 periodo largo
                        numberOfPeriods = 1
                        ReDim aux(0 To numberOfPeriods, 0 To 6)
                        aux(1, 1) = startDate
                        aux(1, 2) = endDate
                    Else
...
#End Region
#Region "Fx Option"
    <ExcelFunction(category:=Categoria, Description:="Esta funcion calcula el valor de una opcion call o put a partir de la formula de Black y Scholes")> _
    Shared Function CP_Black(<ExcelArgument(Description:="Tipo: dice si es call (C) o put (P)")> ByVal tipo As String, _
                                    <ExcelArgument(Description:="T: plazo a vencimiento de la opcion expresado en dias/365")> ByVal T As Double, _
                                    <ExcelArgument(Description:="Strike: precio de ejercicio o strike de la opcion")> ByVal strike As Double, _
                                    <ExcelArgument(Description:="Forward: el precio forward del subyacente al mismo plazo de vencimiento que la opcion")> ByVal Forward As Double, _
                                    <ExcelArgument(Description:="Sigma: la volatilidad a utilizar")> ByVal sigma As Double, _
                                    <ExcelArgument(Description:="df: factor de descuento correspondiente a la tasa en CLP")> ByVal Df As Double) As Double
        ...
        d1 = (Math.Log(Forward / strike) + 0.5 * T * sigma ^ 2) / (Math.Sqrt(T) * sigma)
        d2 = d1 - Math.Sqrt(T) * sigma
        value = Df * (factor * Forward * CND(factor * d1) - factor * strike * CND(factor * d2))

        Return value
    End Function

    <ExcelFunction(category:=Categoria, Description:="Esta funcion calcula el valor de una opcion call o put a partir de la formula de Black y Scholes")> _
    Shared Function CP_Black_SpotDelta(<ExcelArgument(Description:="Tipo: dice si es call (C) o put (P)")> ByVal tipo As String, _
                                    <ExcelArgument(Description:="T: plazo a vencimiento de la opcion expresado en dias/365")> ByVal T As Double, _
                                    <ExcelArgument(Description:="Strike: precio de ejercicio o strike de la opcion")> ByVal strike As Double, _
                                    <ExcelArgument(Description:="Forward: el precio forward del subyacente al mismo plazo de vencimiento que la opcion")> ByVal Forward As Double, _
                                    <ExcelArgument(Description:="Sigma: la volatilidad a utilizar")> ByVal sigma As Double, _
                                    <ExcelArgument(Description:="df: factor de descuento correspondiente a la tasa en USD")> ByVal df_usd As Double) As Double
        ...
        If tipo = "C" Then
            delta = df_usd * CND(d1)
        Else
            delta = df_usd * (CND(d1) - 1)
        End If
        Return delta
    End Function
    <ExcelFunction(category:=Categoria, Description:="Esta funcion calcula el valor de una opcion call o put a partir de la formula de Black y Scholes")> _
    Shared Function CP_Black_ForwardDelta(<ExcelArgument(Description:="Tipo: dice si es call (C) o put (P)")> ByVal tipo As String, _
                                    <ExcelArgument(Description:="T: plazo a vencimiento de la opcion expresado en dias/365")> ByVal T As Double, _
                                    <ExcelArgument(Description:="Strike: precio de ejercicio o strike de la opcion")> ByVal strike As Double, _
                                    <ExcelArgument(Description:="Forward: el precio forward del subyacente al mismo plazo de vencimiento que la opcion")> ByVal Forward As Double, _
                                    <ExcelArgument(Description:="Sigma: la volatilidad a utilizar")> ByVal sigma As Double, _
                                    <ExcelArgument(Description:="df: factor de descuento correspondiente a la tasa en CLP")> ByVal df_clp As Double) As Double
        ...
        If tipo = "C" Then
            delta = df_clp * CND(d1)
        Else
            delta = df_clp * (CND(d1) - 1)
        End If
        Return delta
    End Function

    <ExcelFunction(category:=Categoria, Description:="Cálculo de Vega (derivada del valor de la opcion respecto al parametro sigma)")> _
    Shared Function CP_Black_Vega(<ExcelArgument(Description:="T: plazo a vencimiento de la opcion expresado en dias/365")> ByVal T As Double, _
                                    <ExcelArgument(Description:="Strike: precio de ejercicio o strike de la opcion")> ByVal strike As Double, _
                                    <ExcelArgument(Description:="Spot")> ByVal spot As Double, _
                                    <ExcelArgument(Description:="Forward: el precio forward del subyacente al mismo plazo de vencimiento que la opcion")> ByVal Forward As Double, _
                                    <ExcelArgument(Description:="Sigma: la volatilidad a utilizar")> ByVal sigma As Double, _
                                    <ExcelArgument(Description:="df_usd: factor de descuento correspondiente a la tasa en USD")> ByVal df_usd As Double) As Double
        ...
        Pi = MyPi()
        d1 = (Math.Log(Forward / strike) + 0.5 * T * sigma ^ 2) / (Math.Sqrt(T) * sigma)
        aux = 1 / (Math.Sqrt(2 * Pi)) * Math.Exp(-0.5 * d1 ^ 2)
        vega = spot * Math.Sqrt(T) * aux * df_usd
        Return vega
    End Function

    <ExcelFunction(category:=Categoria, Description:="Cálculo de Gamma")> _
    Shared Function CP_Black_Gamma(<ExcelArgument(Description:="T: plazo a vencimiento de la opcion expresado en dias/365")> ByVal T As Double, _
                                    <ExcelArgument(Description:="Strike: precio de ejercicio o strike de la opcion")> ByVal strike As Double, _
                                    <ExcelArgument(Description:="Spot")> ByVal spot As Double, _
                                    <ExcelArgument(Description:="Forward: el precio forward del subyacente al mismo plazo de vencimiento que la opcion")> ByVal Forward As Double, _
                                    <ExcelArgument(Description:="Sigma: la volatilidad a utilizar")> ByVal sigma As Double, _
        ...
        d1 = (Math.Log(Forward / strike) + 0.5 * T * sigma ^ 2) / (Math.Sqrt(T) * sigma)
        aux = 1 / (Math.Sqrt(2 * Pi)) * Math.Exp(-0.5 * d1 ^ 2)
        gamma = aux * df_usd / (spot * sigma * Math.Sqrt(T))

        Return gamma
    End Function
    <ExcelFunction(category:=Categoria, Description:="Esta funcion hace interpolacion por método de Varianza Total y se aplica a la interpolación de una volatilidad ATM.")> _
    Shared Function VannaVolga(<ExcelArgument(Description:="Tenors")> _
                                        ByVal Tenors As Object, _
                                        <ExcelArgument(Description:="atm")> _
                                        ByVal atm As Object, _
                                        <ExcelArgument(Description:="rr")> _
                                        ByVal rr As Object, _
                                        <ExcelArgument(Description:="vwb")> _
                                        ByVal vwb As Object, _
                                        <ExcelArgument(Description:="Strike")> _
                                        ByVal k As Double, _
                                        <ExcelArgument(Description:="plazo")> _
                                        ByVal plazo As Double, _
                                        <ExcelArgument(Description:="spot")> _
                                        ByVal spot As Double, _
                                        <ExcelArgument(Description:="tenorsClp")> _
                                        ByVal tenorsClp As Object, _
                                        <ExcelArgument(Description:="ratesClp")> _
                                        ByVal ratesClp As Object, _
                                        <ExcelArgument(Description:="tenorsCld")> _
                                        ByVal tenorsCld As Object, _
                                        <ExcelArgument(Description:="ratesCld")> _
                                        ByVal ratesCld As Object, _
                                        <ExcelArgument(Description:="convención")> _
                                        ByVal conven As String) As Double

        ' Esta función es igual vannavolga3 de la planilla Opciones_por_Internet_v2.xls
        'aqui se trata de invertir la relacion c(k)=cbs(k)+x(i)(cmkt(ki)-cbs(ki))
        'para encontrar la vol
        'la inversión la hacemos por un newton rapson
        'Interpolo el smile al plazo que necesito

        Dim atmplazo As Double
        Dim rrplazo As Double
        Dim vwbplazo As Double
        Dim satm As Double
        Dim scall As Double
        Dim sput As Double
        Dim t As Double
        ...
        Dim v2 As Double
        Dim v3 As Double
        Dim df_usd As Double
        Dim df_clp As Double
        Dim valor As Double
        Dim vol As Double
        Dim diff As Double
        Dim dp As Double
        ...
        Try
            ...
            atmplazo = ATMInterpol(Tenors, atm, plazo) 'esta es la linea nueva. se cambia la interpolacion lineal
            'por la interpolacion de varianza total

            rrplazo = LinInterpol(Tenors, rr, plazo)
            vwbplazo = LinInterpol(Tenors, vwb, plazo)

            'Del smile calculo las vols
            satm = atmplazo / 100
            scall = (atmplazo + vwbplazo + 0.5 * rrplazo) / 100
            sput = (atmplazo + vwbplazo - 0.5 * rrplazo) / 100

            'Dadas las vols calculo los strikes
            t = plazo / 365
            raizt = Math.Sqrt(t)
            rclp = LinInterpol(tenorsClp, ratesClp, plazo)
            rcld = LinInterpol(tenorsCld, ratesCld, plazo)

            If conven = "Spot Delta" Then
                alpha = -ICND(0.25 * (1 + rcld) ^ t)
            Else

                alpha = -ICND(0.25 * (1 + rclp) ^ t)
            End If

            fwd = spot * ((1 + rclp) / (1 + rcld)) ^ (t)
            k2 = fwd * Math.Exp(0.5 * t * satm ^ 2)
            k3 = fwd * Math.Exp(0.5 * t * scall ^ 2) * Math.Exp(alpha * scall * raizt)
            k1 = fwd * Math.Exp(0.5 * t * sput ^ 2) * Math.Exp(-alpha * sput * raizt)

            'Calculo los coeficientes
            coef1 = (Math.Log(k2 / k) * Math.Log(k3 / k)) / (Math.Log(k2 / k1) * Math.Log(k3 / k1))
            coef2 = (Math.Log(k / k1) * Math.Log(k3 / k)) / (Math.Log(k2 / k1) * Math.Log(k3 / k2))
            coef3 = (Math.Log(k / k1) * Math.Log(k / k2)) / (Math.Log(k3 / k1) * Math.Log(k3 / k2))

            'Calculo los Vegas
            df_usd = (1 + rcld) ^ (-t)
            v = CP_Black_Vega(t, k, spot, fwd, satm, df_usd)
            v1 = CP_Black_Vega(t, k1, spot, fwd, satm, df_usd)
            v2 = CP_Black_Vega(t, k2, spot, fwd, satm, df_usd)
            v3 = CP_Black_Vega(t, k3, spot, fwd, satm, df_usd)
            ...
            'Calculo el valor de la opcion
            df_clp = (1 + rclp) ^ (-t)
            valor = CP_Black("C", t, k, fwd, satm, df_clp) + coef1 * (CP_Black("C", t, k1, fwd, sput, df_clp) - CP_Black("C", t, k1, fwd, satm, df_clp))
            valor = valor + coef2 * (CP_Black("C", t, k2, fwd, satm, df_clp) - CP_Black("C", t, k2, fwd, satm, df_clp))
            valor = valor + coef3 * (CP_Black("C", t, k3, fwd, scall, df_clp) - CP_Black("C", t, k3, fwd, satm, df_clp))

            vol = satm : diff = 1 : dp = 1.0E+20
            While Math.Abs(diff) > 0.000001
                vol = vol - diff / dp
                p = CP_Black("C", t, k, fwd, vol, df_clp)
                dp = (CP_Black("C", t, k, fwd, vol + 0.0001, df_clp) - p) / 0.0001
                diff = p - valor
            End While
